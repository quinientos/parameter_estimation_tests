...
systems = load_systems('input_files/seir_system.json') 
systems_by_name = {}
for system in systems["systems"]:
    systems_by_name[system["name"]] = system
system = systems_by_name["seir"]    

instances = {"instances":[]}
instance_basename = "seir_orig_"
...
instance_basename = "seir_even_"
true_values = [0.125, 0.25, 0.375, 0.5, 0.625, 0.75, 0.875]
for i in range(10):
    tmp_vals = np.random.permutation(true_values) if i>0 else true_values
    param_values = tmp_vals[:3]
    state_values = tmp_vals[3:]
    instance_name = instance_basename + str(i)
    instance = generate_instance(system, instance_name, param_values, state_values)
    instances["instances"].append(instance)


instance_basename = "seir_rand_"
for i in range(10):
    instance_name = instance_basename + str(i)
    param_values = np.random.rand(1,3).round(3).tolist()[0]
...
with open('input_files/seir_instances.json', 'w') as outfile:
