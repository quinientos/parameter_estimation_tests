...
PLATFORMS = ["amigo2"] #, "iqm", "sciml"]

def compile_results(system, instance, platform, i, csv_writer):
    res_file = "./test_files/"+ platform + "/outputs/{}.csv".format(instance["name"] + "_" + str(i))

    total_num_vars = len(system["parameter-variables"]) + len(system["state-variables"])
    csv_writer.writerow([instance["name"]] + [""]*total_num_vars)
    df = pd.read_csv(res_file)
    if df.empty:
        print("No solution found! {}".format(res_file))
        return
...
def main(args): 
    ...
    for system in systems["systems"]:
        ...
        for platform in PLATFORMS:
            print(platform)
            for upperbound in range(1,4):
                with open("table_files/"+system["name"] + "_results_" + str(upperbound) + "_" + platform + ".csv", "w") as csv_file:
                    csvwriter = csv.writer(csv_file)
                    #write header
                    csvwriter.writerow([""] + system["parameter-variables"] + system["state-variables"])        
                    for i in range(10):
                        instance = instances_by_name[system["name"] + "_" + str(i)]
                        compile_results(system, instance, platform, upperbound, csvwriter)            
